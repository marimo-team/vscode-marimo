...
export async function start({ controller, mode }: { controller: MarimoController; mode: AppMode }) {
  ...
  if (controller.currentMode === mode && controller.active) {
    ...
    window
      .showInformationMessage(`${controller.appName} is already running in ${mode} mode`, button)
      .then((response) => {
        if (response === button) {
          controller.open();
        }
